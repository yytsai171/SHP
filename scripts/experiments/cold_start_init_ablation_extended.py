"""
cold_start_init_ablation_extended.py
=======================================
Extends cold_start_init_ablation.py's k=10/k=100 probes to the two
elicitation budgets it did not test, k=25 and k=50, using the identical
methodology (same 200-user tuning subset, same three init strategies,
same SHLCP reference strategy).

Motivation: cold_start_init_ablation.py found a genuine crossover
(item-init wins at k=10, zero-init wins at k=100) but never tested
k=25/50, so the location of the crossover point is not actually known
-- assuming it falls on one side or the other without data would be a
guess, not a finding. This script closes that gap so the per-k init
choice is grounded in evidence at all four k values used throughout
this study, rather than at only two of them.

Reference strategy: SHLCP, same 200-user tuning subset as
cold_start_init_ablation.py -- results are directly comparable and
appended to the same three init categories ('zero', 'item',
'top5_mean').

Usage
-----
    python scripts/experiments/cold_start_init_ablation_extended.py
...
Output
------
    results/cold_start_init_ablation_extended_results.csv
        Columns: init, k, val_rmse, n_users, seconds
        (k in {25, 50} only -- combine with
        cold_start_init_ablation_results.csv for the full {10,25,50,100}
        picture.)
"""
...
OUT_INIT_ABL_EXT: str = os.path.join(RESULTS_DIR, 'cold_start_init_ablation_extended_results.csv')

INIT_STRATEGIES: List[str] = ['zero', 'item', 'top5_mean']
# The two k values cold_start_init_ablation.py did not test.
K_PROBES: List[int] = [25, 50]
...
def main() -> None:
    ...
    # identical to cold_start_init_ablation.py.
    ...
    def partial_lfm_update_cold(
        ...
        """One partial-SGD update, identical to the model's default
        update -- only the initialisation of pu_cold (before any
        update is applied) is under test in this ablation."""
    ...
    print(f"\n=== Cold-start initialisation ablation (EXTENDED, k=25/50): SHLCP, "
    ...
    print(f"\n{'='*70}", flush=True)
    print("=== Best init per k (extended: 25, 50) ===", flush=True)
    for k in K_PROBES:
    ...
    df.to_csv(OUT_INIT_ABL_EXT, index=False)
    print(f"\nSaved to {OUT_INIT_ABL_EXT}", flush=True)
